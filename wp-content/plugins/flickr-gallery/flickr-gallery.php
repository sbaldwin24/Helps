											...
											<div class="flickr-photos">
												<?php foreach ( $pager->get(1) as $key => $photo ) : ?>
													<div class="flickr-thumb">
														<a href="<?php echo $url . $photo['id'] ?>" target="_blank"><img class="<?php echo $photo['media'] ?>" title="<?php echo str_replace("\"", "\\\"", $photo['title']) ?>" src="<?php echo $phpFlickr->buildPhotoURL($photo, 'square') ?>" alt="<?php echo str_replace("\"", "\\\"", $photo['title']) ?>" /></a>
													</div>
												<?php endforeach; ?>
											</div>
	...
	function ajax_pagination() {
		...
		if ( !is_null($pager->_extra) ) {
			...
			foreach ( $pager->get($_POST['page']) as $key => $photo ) : 
				$html .= '<div class="flickr-thumb"><a href="http://flickr.com/photo.gne?id=' . $photo['id'] . '" target="_blank"><img class="' . $photo['media'] . '" title="' . str_replace("\"", "\\\"", $photo['title']) . '" src="' . $phpFlickr->buildPhotoURL($photo, 'square') .'" alt="' .  str_replace("\"", "\\\"", $photo['title']) .'" /></a></div>';
			endforeach;
		}
		...
		?>
			<div class="flickr-photos">
				<?php foreach ( $pager->get($page) as $key => $photo ) : ?>
					<div class="flickr-thumb">
						<a href="http://flickr.com/photo.gne?id=<?php echo $photo['id'] ?>" target="_blank"><img class="<?php echo $photo['media'] ?>" title="<?php echo str_replace("\"", "\\\"", $photo['title']) ?>" alt="<?php echo str_replace("\"", "\\\"", $photo['title']) ?>" src="<?php echo $phpFlickr->buildPhotoURL($photo, 'square') ?>" /></a>
					</div>
				<?php endforeach ?>
				<div class="fg-clear"></div>
			</div>
